...
class APIController {
    ...
    static func getAnalyzation(queryParams: [String: String] = [:], _ onComplete: @escaping (Result<AnalyzationResponse, ApiError>) -> Void) {
        APIRequest.execute(ApiEndpoint(.analyze, queryParams), .get, decodable: AnalyzationResponse.self, onComplete: onComplete)
    }
